...
| Roblox dashboard docking | Prototype | Macro can pin a verified Roblox player into a DPI-aware `1366 x 700` live surface; a tracked post-rejoin window is repaired in place if Roblox rewrites its dock style, process replacement is reacquired, docking is suspended while LilacMacro is behind another application, and original frame/bounds are restored on undock, tab change, minimize/occlusion, failure, or shutdown; live behavior remains owner-validated |
