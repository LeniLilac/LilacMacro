...
| Full Settings page | Prototype | Press-then-key bindings, Plan state, visible-but-DPAPI-protected private-server link, masked DPAPI webhook, Discord failure options, full/compact display policy, coordinated update controls, and the local instance manager persist; private-server Test Link uses the registered Roblox protocol, while webhook delivery and most remaining Discord effects remain unwired |
