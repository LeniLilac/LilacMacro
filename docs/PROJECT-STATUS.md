...
| Roblox dashboard docking | Prototype | Macro can pin a verified Roblox player into a DPI-aware `1366 x 700` live surface; a tracked post-rejoin window is repaired in place if Roblox rewrites its dock style, a foreground replacement process is reacquired once and maintained by the same exposure rule, overlapping unrelated applications suspend docking, and original frame/bounds are restored on undock, tab change, minimize/occlusion, failure, or shutdown; live behavior remains owner-validated |
...
| Full Settings page | Prototype | Press-then-key bindings, Plan state, persisted mode/color themes, visible-but-DPAPI-protected private-server link, masked DPAPI webhook, Discord failure options, full/compact display policy, coordinated update controls, and the local instance manager persist; private-server Test Link uses the registered Roblox protocol and Test Webhook sends a bounded mention-free Discord message, while runtime-triggered Discord delivery remains unwired |
