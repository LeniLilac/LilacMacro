...
| Full Settings page | Prototype | Press-then-key bindings, Plan state, masked DPAPI private-server/webhook values, Discord failure options, and the local instance manager persist; private-server Test Link uses the registered Roblox protocol, while webhook delivery, updates, and most remaining General/Discord effects remain unwired |
| Local instance manager | Prototype | Exact-binary TermWrap preflight/cache, loopback isolation, per-runner standard accounts/credentials/ACLs/full-UI tasks, Runner 1..16 endpoints, shared-or-isolated configuration roots, legacy single-runner migration, rollback, cleanup verification, and Settings setup/add/open/remove actions exist; disposable-VM lifecycle certification and broader owner live acceptance remain required |
...
| Private-server links and webhooks | Prototype | Masked settings and current-user DPAPI persistence are implemented; private-server protocol launch is bounded and validated, while webhook validation/delivery remains Planned |
