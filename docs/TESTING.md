...
| Discord webhook delivery | Stable/PTB/Canary official-host allowlist, lookalike-host rejection, Components V2 shape, per-event persistence, explicit terminal-failure user allowlist, other-mention suppression, bounded background delivery, secret-free transport/HTTP failures, full suite | Owner tests a webhook copied from each Discord client in use, toggles each event, and confirms compact start/stop, task, win/loss, recovery, and terminal-failure messages. Only terminal failure may mention the configured user; automation timing remains unchanged during a slow or unavailable webhook |
