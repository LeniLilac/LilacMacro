...
| Roblox dashboard dock | Style/exposure/activation/maintenance/work-area policy tests, including tracked post-rejoin style repair, direct dashboard-to-Roblox focus authorization, and rejected background reacquisition after suspension, warning-free build, full suite | Owner validates exact `1366 x 700` client size, interactive dock, macro input focus and post-rejoin stability without alternating dock/undock, then backgrounds both windows, focuses another app, focuses standalone Roblox, and confirms no dock/undock loop until LilacMacro is activated; also validates taskbar-safe maximize, undock restoration, tab/minimize behavior, and shutdown cleanup |
