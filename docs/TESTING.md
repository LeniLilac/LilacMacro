...
| Roblox dashboard dock | Style/exposure/activation/maintenance/work-area policy tests, including tracked post-rejoin style repair and direct dashboard-to-Roblox focus authorization, warning-free build, full suite | Owner validates exact `1366 x 700` client size, interactive dock, macro input focus and post-rejoin stability without alternating dock/undock, background-app focus behavior, taskbar-safe maximize, undock restoration, tab/minimize behavior, and shutdown cleanup |
