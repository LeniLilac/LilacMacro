...
| UI-scale normalization | Synthetic panel ownership across `0.80`-`1.20`, semantic OCR without numeric text, feedback/clamp boundaries, per-session cache isolation, supplied-dataset validation, warning-free build, full suite | Owner starts at several numeric values on the desktop and local-runner/RDP session, confirms both converge to the same rendered panel size and Lobby, then changes a previously cached environment and confirms stale-cache recalibration |
