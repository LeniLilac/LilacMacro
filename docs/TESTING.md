...
| Deep debug | Archive entries, camel/snake-case JSONL parsing, redaction, input-marker coordinate translation, surface-specific retention, independent cleanup-option persistence, single-owner behavior, full suite | Owner confirms Runtime Lab shows `DEEP DEBUG ON`, a run longer than 15 minutes retains its early frames, disabling local cleanup preserves more than 20 completed ZIPs, re-enabling it prunes to the newest 20 without enabling uploads, archives open or drop into the dedicated viewer, nearby events align, click/scroll markers land correctly, and malformed entries fail visibly |
| Privacy, telemetry, and diagnostic upload | First-run notice-version persistence and independent choices; no pre-acceptance network start; online control/update gating; fixed telemetry schema, OCR/local-instance failure code and scope bounds, post-success rate limiting, size/text bounds, exact endpoint, redirect rejection, no raw diagnostic strings; light-buffer frame/event/byte bounds and safe-field selection; stable random installation identity; trusted B2 origin/prefix and lookalike rejection; manual and automatic cleanup semantics; warning-free build; full suite | Owner reviews first-run defaults, saves each combination, and confirms Settings reflects it. With a disposable test endpoint, confirm telemetry produces only aggregate rows; induce classified OCR and local-instance failures and confirm one bounded event with no command output, then retry and confirm the ledger suppresses each; confirm those events alone do not trigger an automatic report; trigger one light report with Deep Debug off and one archive with it on; confirm temporary light cleanup, successful Deep Debug cleanup, failed Deep Debug retention, and that all three disabled choices produce no covered requests. Never use an owner capture for staging validation |
...
| Roblox dashboard dock | Style/exposure/maintenance/work-area policy tests, including tracked post-rejoin style repair, explicit rejoin reacquisition, inactive-owner minimize policy, and user-focused Roblox preservation, warning-free build, full suite | Owner validates exact `1366 x 700` client size, interactive dock, macro input focus and post-rejoin redocking without alternating dock/undock; switches to Plan and confirms Roblox minimizes, focuses Roblox and confirms it stays standalone, then focuses the Macro and confirms one stable redock. Also validates foreign-window focus, taskbar-safe maximize, undock restoration, and shutdown cleanup |
...
| Discord webhook delivery | Stable/PTB/Canary official-host allowlist, lookalike-host rejection, Components V2 shape, multipart PNG attachment and Media Gallery shape for every real event, text-only Test webhook, per-event persistence, explicit terminal-failure user allowlist, other-mention suppression, bounded background delivery, capture-failure fallback, secret-free transport/HTTP failures, full suite | Owner tests a webhook copied from each Discord client in use, toggles each event, and confirms compact start/stop, task, win/loss, recovery, and terminal-failure messages show the current Roblox client image when capture is available. Test webhook remains text-only. Only terminal failure may mention the configured user; automation timing remains unchanged during a slow, unavailable, or uncapturable webhook |
...
| Coordinated application update | Exact tag/release/six-asset/GitHub-digest/checksum/Ed25519 policy tests, trusted-redirect tests, manifest/signature tamper negatives, state/request round trips, installer-script checks, warning-free build, full suite | From an older Program Files build, owner downloads and installs a newer official public-beta release, confirms Windows shows the documented unknown publisher, every open UI closes normally, active binaries are never overwritten, owner/runner UIs relaunch at the new version, and cancellation/tampering/private-release cases fail closed |
