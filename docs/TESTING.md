...
| Optional diagnostic upload | Default-off consent persistence; stable random installation identity and corrupt-identity fail-closed behavior; trusted B2 origin/prefix and lookalike rejection; 3/30-GiB policy boundaries; whole-file and per-part SHA-256/length binding; exact completion state; cancellation; warning-free build; full suite | Owner enables diagnostic upload in Settings, selects one small test ZIP, confirms progress reaches server verification, then disables consent and confirms no upload action is available. Large-file and production storage validation belongs to disposable staging infrastructure, not owner captures |
