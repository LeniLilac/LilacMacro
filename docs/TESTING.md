...
| Roblox dashboard dock | Style/exposure/maintenance/work-area policy tests, including tracked post-rejoin style repair and foreground-source acquire-to-maintain stability, warning-free build, full suite | Owner validates exact `1366 x 700` client size, interactive dock, macro input focus and post-rejoin redocking without alternating dock/undock, then backgrounds both windows, focuses another app, focuses standalone Roblox, and confirms one stable redock rather than a loop; also validates taskbar-safe maximize, undock restoration, tab/minimize behavior, and shutdown cleanup |
...
| Discord webhook delivery | Stable/PTB/Canary official-host allowlist, lookalike-host rejection, mention suppression, secret-free transport/HTTP failures, full suite | Owner tests a webhook copied from each Discord client in use and confirms delivery without exposing the URL in status or errors |
