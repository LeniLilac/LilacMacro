...
| Macro terminal continuation | Exact-task Repeat Stage policy positive/negative matrix, warning-free build, full suite | Owner runs a multi-win Story/Raid task and confirms the result clicks Repeat Stage, verifies Match Prestart, and reruns placements without opening Teams or aligning the camera; then confirms reaching the target or selecting another task/act/mode resets through Lobby, and Challenge never repeats |
...
| Resource refuel tasks | Route identity/order, exact field walk timing, dialog geometry boundaries, station/dialog OCR ownership, Escape key input mapping, scheduler handoff, snapshot validation, warning-free build, full suite | Owner configures Mine Refuel, Drill Refuel, then Raid; confirms one startup normalization, Mine refuel and Esc/R/Enter Lobby return, Drill refuel and Lobby return, then direct Raid navigation. Also confirms Mine/Drill keep independent recurrence times and Min/Max label changes do not affect the relational quantity click |
...
| Expedition runtime | Node tracker real-Boss transitions, delayed-arrival policy, tooltip parsing negatives, dataset-owned Hover Line initial sweep, cached marker-relative hover/local reacquisition, multi-scale marker location, personalized color ambiguity, resource-scoped quantity normalization, reward-card association, Route Optimizer trial bounds, separate Encounter/Checkpoint source and confirmation ROI ownership, duplicate-label rejection, retained-source temporal retry, current Start Game evidence, snapshot validation, warning-free build, full suite | Owner runs Route Optimizer for at least three trials at each UI scale and confirms every row reports the observed quantity, threshold decision, raw OCR, and elapsed time; confirms non-final trials reroll and the final trial remains at Match Prestart; then validates initial placement/immediate spawn Continue, first-node left-to-right hover calibration, later cached-offset node actions, extraction, idle reward-card dismissal, cancellation, and recovery. For Encounter and a non-spawn Checkpoint, confirm node classification can occur while the ship moves without causing input, then verify Continue is clicked only after arrival exposes the source control. Delay one confirmation click and confirm the modal action is retried from the modal, while a closed confirmation that returns to the source reopens after stable fresh evidence rather than succeeding or clicking the dimmed background control. |
