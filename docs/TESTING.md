...
| Capture/window/input | Pure geometry/work-area/protocol regression tests, including taskbar, offset-monitor, exact-width, and too-small-work-area cases; full suite | Owner moves canonical Roblox partly under the taskbar and beyond each monitor edge, runs an input-owning action, and confirms the full client is brought into usable view before input; also validates sizing, focus, capture freshness, cancellation, and cleanup |
