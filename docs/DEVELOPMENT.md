...
| Macro settings | `%LOCALAPPDATA%\LilacMacro\macro-settings.json` normally; `%ProgramData%\LilacMacro\Configurations\shared` for the local-instance provisioning owner and shared runners | Schema-versioned, cross-process-serialized atomic replacement for display/update preferences, keybinds, Plan snapshots/selection/reporting options, and DPAPI ciphertext for private-server/webhook values; `privacy-choices.json` independently owns the authoritative monotonic consent generation so stale ordinary saves cannot re-enable a choice; the shared ProgramData root uses machine-scope DPAPI plus an owner/runner ACL, and unrelated Windows profiles fail closed to their own local root |
...
| Deep debug | `%ProgramData%\LilacMacro\Diagnostics` after local-instance provisioning; otherwise `%LOCALAPPDATA%\LilacMacro\diagnostics` | Bounded ZIP archives plus transient staging; the environment snapshot records the complete Windows build including the registry UBR revision; one archive-byte budget is atomic and machine-wide for owner/runners, fixed one-second capture retains all evidence by using decode-verified quality-14 JPEG for older ordinary frames and pixel-exact AVIF for important frames before evicting only enough low-priority evidence to fit 3 GiB, capture pauses below 3 GiB free, sent logs remain until byte-budget pruning, and old profile-local archives are not migrated |
