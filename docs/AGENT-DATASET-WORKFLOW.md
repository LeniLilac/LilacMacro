...
5. Inspect `crops/` for labeled regions and `ocr-maps/` for source-to-text comparisons. A crop path and every nested OCR line rectangle are joined to their annotation in `frames.jsonl`.

OCR child rectangles use the same original-frame coordinate space as their manual parent rectangle. Each trial includes its `device`; `total_compute_milliseconds` includes model load plus inference, and `model_was_cached` distinguishes a resident model/device pipeline from a cold run. OCR output is advisory and does not establish UI ownership on its own.
