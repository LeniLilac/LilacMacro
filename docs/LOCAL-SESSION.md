...
`+ ADD SHARED` points the runner at `%ProgramData%\LilacMacro\Configurations\shared`. Only the Windows account recorded as the provisioning owner switches its normal desktop UI to that root; an unrelated Windows profile on the same machine keeps its own `%LOCALAPPDATA%\LilacMacro` configuration. Plan definitions, selected Plan, keybinds, private-server/webhook settings, and placements are common to the provisioning owner's UI and every shared runner. Existing owner settings and placements seed the shared root once. Secret values are reprotected with machine-scope DPAPI and the directory ACL admits only the provisioning owner, selected shared runners, SYSTEM, and Administrators. Macro-settings writes take a cross-process file lock before atomic replacement.

`+ ADD SEPARATE` gives the runner `%ProgramData%\LilacMacro\Configurations\runner-N`, with an ACL limited to the owner, that runner, SYSTEM, and Administrators. Its Plan, keybind, secret, and placement edits do not affect any other instance. Runtime state, OCR environments, UI-scale caches, crash logs, Roblox settings, and Roblox credentials remain inside the Windows account/session that owns the UI; they are never shared configuration. Deep Debug is the deliberate exception: all configured runners and the provisioning owner write to the protected `%ProgramData%\LilacMacro\Diagnostics` archive pool.
