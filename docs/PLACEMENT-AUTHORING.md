...
**Status: Prototype.** Setup can discover maps, edit per-route timelines, and autosave them. Runtime Lab can execute a selected Story/Raid/Challenge route with bounded evidence; unattended main-macro playback remains Planned.
...
|---|---|
| Story | School Grounds, Flower Forest, Rose Kingdom, Fairy King Forest, and King's Tomb; King's Tomb combines two compatible reference datasets |
| Raid | Spirit City Act 1, Act 2, and Act 3 as separate cards |
| Expedition | School Grounds, Flower Forest, and Rose Kingdom |
...
A left-click on the map immediately creates a Place action at the original-image pixel coordinate using the selected Unit Slot and route defaults. Each point is rendered as a compact dot, collision-aware connector, and unit-slot label. A slot used once is labeled `1`; repeated placements of that slot are labeled `1a`, `1b`, and so on in timeline order. Hovering the dot exposes the move cursor; left-dragging it previews a new point and atomically saves the final original-image coordinate. The stable placement ID does not change, so dependent Reconfigure, Upgrade, and Sell actions continue to reference the moved point. Invalid drops fail validation and restore the saved position.

`Add Step` opens one centered owner-modal editor for Delay, Reconfigure, Upgrade, and Sell plus the fields for the selected action; Place is not offered there. `Edit` opens the same styled field surface for that row. Rows reorder by dragging their grip handle; no separate up/down controls are exposed. The step kind is shown by a colored action rail and title, with separate light- and dark-palette colors. Position around the Start Game boundary conveys before/after state, so phase chips are not shown. Selection uses a themed surface and ink outline rather than native ListBox selection chrome.
...
**Prototype:** Runtime Lab maps authored image points to the verified 1366 by 700 client, batches contiguous Place steps through Quick placement, retries only failed rows, verifies selected units, executes the Start Game boundary, and supports Delay, Reconfigure, Upgrade, and Sell. It stops on ambiguity or cancellation. Main priority-scheduler ownership and owner live acceptance remain Planned.

Current authoring does not:

- identify the live map or camera pose;
- click units or placement points in Roblox;
- detect affordability, placement success, upgrade state, or sale completion;
- execute delays or target changes;
