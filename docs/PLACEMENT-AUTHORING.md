...
Roblox may automatically start the match while prestart actions are still executing. Playback does not cancel or reclassify those actions: it finishes every authored prestart action in order. At the `Start Game` boundary it first attempts the normal fresh Start-screen action. Three consecutive Start-screen misses cause playback to reselect one active authored placement; fresh physical selected-unit evidence satisfies the boundary as an automatic start, and that panel is dismissed again before playback continues with the authored after-start actions. A missing Start screen without that independent live evidence remains indeterminate and fails closed.
