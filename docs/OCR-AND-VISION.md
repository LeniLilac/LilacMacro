...
- Requests and responses use a unique temporary channel, worker-exit detection, bounded JSON, and cleanup. Each worker publication uses a unique temporary file and bounded Windows access-denied retries before atomic replacement. The worker publishes fixed lifecycle phases for input validation, crop preparation, model loading, inference, and response writing so Deep Debug can distinguish the stage of a stall without recording input paths. Cold model loading, including the first PaddleOCR module import, is published before that work begins and has a separate two-minute bound; every warm or non-model phase retains the 30-second watchdog. Readers share status/response files with atomic replacement, and workers ignore inherited Python runtime paths and user-site packages. A virtual environment whose Python home was persisted with invalid surrounding quotes is rejected until Repair rebuilds it.
