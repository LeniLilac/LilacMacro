...
- Discord runtime events use a bounded single-reader background queue and official Components V2 containers. Real events attach a bounded current Roblox client PNG when capture is available; the text-only Test webhook path does not. Event-specific settings gate run starts/stops, task changes, results, recovery attempts, and terminal failures; only the terminal-failure path can allowlist the configured Discord user ID for a mention.
...
App owns one process-wide deep-debug recorder because it coordinates WPF lifecycle, Workspace capture, OCR, vision, and input evidence. A bounded single-reader channel serializes events and PNG bytes. Each active app surface registers a full-client capture provider; the recorder invokes that provider at the persisted capture interval and labels the resulting `live-client` frame as an intentional periodic sample. The complete text timeline is always archived under `%LOCALAPPDATA%\LilacMacro\diagnostics`; Main Macro and Dataset Builder keep the configured final rolling image window, while Runtime Lab keeps images for the complete owner-triggered operation. Both app surfaces consume the same format; Core and Windows do not depend on diagnostics. See [Deep debug](DEEP-DEBUG.md).
