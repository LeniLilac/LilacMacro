...
- Discord runtime events use a bounded single-reader background queue and official Components V2 containers. Event-specific settings gate run starts/stops, task changes, results, recovery attempts, and terminal failures; only the terminal-failure path can allowlist the configured Discord user ID for a mention.
