...
App owns one process-wide deep-debug recorder because it coordinates WPF lifecycle, Workspace capture, OCR, vision, and input evidence. A bounded single-reader writer channel serializes events and PNG bytes away from detection/capture. Each active surface supplies one full-client sample per fixed one-second interval. The writer keeps the newest ten seconds as PNG, converts older ordinary evidence to decode-verified quality-14 JPEG through the in-process Windows imaging component, and converts important evidence to AVIF only after a pixel-exact round trip. A shared file lease serializes only the below-normal-priority native lossless AVIF encoder across owner/runner processes; failures preserve PNG. JPEG and AVIF are stored without redundant ZIP recompression. The recorder retains the complete frame stream until compression cannot keep the staging data or completed ZIP below 3 GiB, then removes only enough evidence to fit, evicting oldest lossy ordinary frames before transition frames and redundant or lower-severity classified-error windows. Error windows protect ten seconds before/after classified failures and merge when they overlap; deterministic signatures and coarse perceptual hashes preserve terminal, first-signature, visually distinct, and recent evidence ahead of repeated near-identical errors. Archive finalization runs away from the WPF dispatcher, and a streaming single-pass artifact rewrite updates PNG references after conversion without work proportional to frame count times log length. After local-instance provisioning, all owner/runner processes archive into ACL-protected `%ProgramData%\LilacMacro\Diagnostics` and enforce one atomic machine-wide byte budget; unprovisioned profiles use `%LOCALAPPDATA%\LilacMacro\diagnostics`. App selects the storage root, Windows provisions its ACL, and Core remains independent of diagnostics. See [Deep debug](DEEP-DEBUG.md).
