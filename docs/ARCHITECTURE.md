...
App owns one process-wide deep-debug recorder because it coordinates WPF lifecycle, Workspace capture, OCR, vision, and input evidence. A bounded single-reader channel serializes events and PNG bytes. Each active surface supplies one full-client sample per fixed one-second interval. The recorder retains the complete frame stream until raw frame evidence crosses the 3 GiB archive-pressure threshold or the completed ZIP exceeds the same hard limit. It then removes only enough evidence to fit, evicting oldest ordinary frames before transition frames and redundant or lower-severity classified-error windows. Error windows still protect ten seconds before/after classified failures and merge when they overlap; deterministic signatures and coarse perceptual hashes preserve terminal, first-signature, visually distinct, and recent evidence ahead of repeated near-identical errors. After local-instance provisioning, all owner/runner processes archive into ACL-protected `%ProgramData%\LilacMacro\Diagnostics` and enforce one atomic machine-wide byte budget; unprovisioned profiles use `%LOCALAPPDATA%\LilacMacro\diagnostics`. App selects the storage root, Windows provisions its ACL, and Core remains independent of diagnostics. See [Deep debug](DEEP-DEBUG.md).
