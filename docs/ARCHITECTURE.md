...
Windows Graphics Capture owns fresh-frame acquisition. For an SDR output it requests the compositor-normalized BGRA8 surface directly, matching the already tone-mapped desktop image used by ordinary screenshots instead of interpreting a driver-provided floating surface as linear scene light. Some per-window Auto HDR paths nevertheless return persistently overexposed BGRA8 content while both DXGI and DisplayConfig still report SDR. The capture owner therefore keeps a bounded 24-frame luminance/clipping window, excludes blank and near-white transition frames, and probes scRGB only when the window's median clipping and P95 luminance match that observed defect. The probe is accepted only when the alternate frame remains structurally correlated, materially reduces clipping and exposure, and is not an unrelated or near-black transition; otherwise BGRA8 remains authoritative, with at most two probes per Roblox process. An accepted probe uses Windows' 203-nit SDR-in-HDR reference and remains the process-scoped capture format. For an HDR or advanced-color output it keeps the source in 16-bit floating-point scRGB, identifies the Roblox monitor through DXGI, reads that display's current SDR white level through DisplayConfig, and normalizes Windows' HDR desktop white boost before tone mapping. The measured white level can establish advanced-color conversion even when DXGI reports the SDR color-space enum. If Windows reports an advanced color space but omits its white measurement, conversion uses the same Windows reference rather than leaving the boost uncorrected. One luminance shoulder compresses HDR highlights and one luminance-preserving gamut operation brings extended colors into sRGB; the old per-pixel brightest-channel normalization is not used. Encoded PNGs carry an explicit sRGB chunk. Format choice, output color context, exposure-probe outcome/source/candidate statistics, structural correlation, and bounded frame luminance/clipping summaries are retained in Deep Debug so a capture-path exposure defect can be distinguished from game artwork. Capture sessions include the Roblox process identity so a replacement process cannot inherit the prior target. Free-threaded frame callbacks remain notification-only for Windows 10 compatibility, and teardown waits for already-dispatched callbacks before releasing WinRT/D3D resources. An incoming compositor-size change recreates the frame pool from that observed size through four bounded attempts and recalculates the client crop; a surface that cannot be mapped still stops without returning assumed geometry, after which the outer owner revalidates the HWND and live bounds through three bounded attempts. Closed or timed-out scRGB sessions are rebuilt once; if the rebuilt session also fails, one final bounded BGRA8 compatibility capture is attempted and retained for that Roblox process, with the fallback identified separately from normal SDR capture in diagnostics.

Full dataset capture copies only the verified client rectangle into a compact GPU texture before CPU readback. The detector path accepts bounded client-relative rectangles, rejects invalid or excessive requests, shelf-packs them into one compact atlas in the active capture format, performs one GPU copy per rectangle and one atlas readback, then splits and converts only the requested pixels to grayscale. `VisualAnchorRegionMatcher` calculates the required search rectangle and translates ROI-local match results back into client coordinates. This path is implemented infrastructure; the unattended runtime that schedules profiles and authorizes input remains Planned.
...
- Settings groups General, Roblox, OCR, Discord, keybind, and diagnostics controls behind internal category tabs. OCR persists Auto/GPU/CPU ownership, runs a deterministic bundled self-test, and repairs only the selected local runtime. General exposes the stable random diagnostic installation ID as a read-only copied support identifier. Theme, the versioned privacy choices, deep-debug diagnostics, and automatic error reports are connected; manual diagnostic uploads are absent. The first Macro launch blocks all choice-covered requests until the current notice is durably saved. A separately atomic `privacy-choices.json` owns a monotonic consent generation across owner and shared-runner processes; covered sends recheck that record, generation changes fail closed for stale processes, and ordinary settings saves merge rather than overwrite it. Local opt-outs revoke immediately, opt-ins activate only after a successful atomic write, and failed writes remain visibly unsaved and fail closed. The same three choices remain editable in General settings.
...
App owns one process-wide deep-debug recorder because it coordinates WPF lifecycle, Workspace capture, OCR, vision, and input evidence. A bounded single-reader writer channel serializes events and PNG bytes away from detection/capture. Each active surface supplies one full-client sample per fixed one-second interval. A separate background optimizer leaves the newest ten seconds as PNG, converts stable ordinary evidence to decode-verified quality-14 JPEG through the in-process Windows imaging component, and converts stable important evidence to AVIF only after a pixel-exact round trip. It rechecks importance before replacing a PNG, so an in-flight ordinary encode cannot make error evidence lossy. A shared file lease serializes only the below-normal-priority native lossless AVIF encoder across owner/runner processes; failures preserve PNG. Stop drains this optimizer for at most ten seconds and then archives remaining PNG evidence. PNG, JPEG, and AVIF are stored without redundant ZIP recompression. The recorder retains the complete frame stream until compression cannot keep the staging data or completed ZIP below 3 GiB, then removes only enough evidence to fit, evicting oldest lossy ordinary frames before transition frames and redundant or lower-severity classified-error windows. Error windows protect ten seconds before/after classified failures and merge when they overlap; deterministic signatures and coarse perceptual hashes preserve terminal, first-signature, visually distinct, and recent evidence ahead of repeated near-identical errors. Archive finalization runs away from the WPF dispatcher, and a streaming single-pass artifact rewrite updates PNG references after conversion without work proportional to frame count times log length. After local-instance provisioning, all owner/runner processes archive into ACL-protected `%ProgramData%\LilacMacro\Diagnostics` and enforce one atomic machine-wide byte budget; unprovisioned profiles use `%LOCALAPPDATA%\LilacMacro\diagnostics`. App selects the storage root, Windows provisions its ACL, and Core remains independent of diagnostics. See [Deep debug](DEEP-DEBUG.md).

Diagnostic transport remains separate from Deep Debug capture. Core owns the single 3 GiB archive limit, deterministic filename and kind validation, and trusted-storage policy. Runtime owns the WPF-free HTTPS, SHA-256, and multipart protocol plus a random installation-identity store. Each upload request sends the exact application version and complete Windows version description alongside the random installation UUID so retained metadata can be searched without downloading private archive contents; the service immediately replaces the UUID with rotating diagnostic pseudonyms. App owns the default-on automatic-error-upload choice, consent-generation cancellation, and lifecycle disposal. Only completed Deep Debug archives containing classified failures are submitted; there is no light archive or in-memory report buffer. Disabling Deep Debug or entering its low-disk pause also pauses automatic uploads. Successfully sent and failed archives both remain local until the shared configured byte budget removes the oldest archives. The app has no manual diagnostic file picker or upload control.

Product telemetry is a separate fixed-schema path. Core owns a discriminated per-kind allowlist and realistic value bounds; Runtime owns a redirect-disabled HTTPS transport to the exact official endpoint plus a small failure rate-limit store; Windows reduces CPU/GPU descriptions to recognized-vendor model names and reads primary-display pixel dimensions without collecting device identifiers. GPU discovery combines session-visible DXGI adapters with the installed display-adapter registry so RDP and isolated local sessions do not lose the physical model when DXGI exposes only the remote adapter. App maps an allowlist of non-frame process observations into bounded events and holds at most 256 in memory. OCR timings carry only the active CPU/GPU category and normalized model, while UI-scale calibration carries bounded display dimensions and input/rendered values. OCR setup failures use classified code/stage/device/runtime flags, and local-instance failures use classified operation/status/mode fields; none carries setup output, account names, profile identifiers, endpoints, serial numbers, or exception text. The client ledger marks a failure code/scope/version only after successful delivery; it is not a disk retry queue. Telemetry never includes arbitrary diagnostic data or causes PNG cloning, and the versioned telemetry choice gates collection and delivery independently from online control and diagnostics.
...
OCR is an optional local Python helper rather than an in-process dependency. Official builds bundle CPU OCR; the setup script creates an isolated environment under `%LOCALAPPDATA%\LilacMacro\ocr` for the optional GPU runtime. Worker processes discard inherited Python home/path/user-site variables before launch so an unrelated system Python cannot contaminate the bundle. Status and response IPC readers permit the worker's atomic file replacement, model-loading phases receive a separate bounded two-minute cold-start deadline, and every other phase retains the 30-second watchdog. CPU workers disable MKL-DNN/oneDNN for the bundled PP-OCRv6 path because the bundled CPU build has an unsupported detection attribute in that backend; GPU workers retain their accelerated path.
...
Runtime also owns a positional in-game option normalizer for the post-scale canonical panel. Settings reopening resets every tab to its top position, so the normalizer verifies tab selection and green/red toggle surfaces, applies the visible Units options, then moves the cursor to the canonical client center and uses one five-second 5,000-unit downward scroll burst before applying the lower options. It does not use scrollbar pixels as state ownership, never OCRs individual option labels, and never treats static points alone as authority. Runtime Lab exposes the complete sequence explicitly, while production invokes it only at Macro start.
...
- Roblox settings access failures are terminal rather than rejoin-retried. Their sanitized error records the failed mutation, file attributes, and Windows HRESULT without exposing the user-profile path.
...
The owner UI coordinates updates rather than copying binaries into live runner profiles. With automatic checks enabled, the main desktop checks at startup, every 30 minutes, and after a run ends or is stopped; managed runners never poll independently. A verified available version produces one persisted, centered, dismissible prompt per version only while no run is active. Core validates one exact six-asset GitHub release contract and the embedded Ed25519 release-trust key; App performs bounded exact-origin downloads, GitHub digest checks, detached project-signature verification, checksum comparison, and records the exact macro PIDs and runner profiles to stop. The intentionally non-Authenticode installer requests ordinary shutdown from every UI, refuses to overwrite an active or uninspectable process, installs once under Program Files, then the allowlisted elevated helper re-registers and launches every configured runner task. Managed runner UIs observe the same machine request and cannot initiate downloads or installation.
