...
Diagnostic transport remains separate from recording. Core owns deterministic size, filename, kind, and trusted-storage policy. Runtime owns the WPF-free HTTPS, SHA-256, and multipart protocol plus a random installation-identity store. App owns the independent manual-upload and automatic-report choices, explicit file selection, bounded in-memory light buffer, progress, cancellation, and lifecycle disposal. Manual upload never enumerates diagnostics. Automatic reporting subscribes to frame observations only while enabled, packages at most 32 already-acquired frames into an 80 MiB temporary ZIP after an error, or submits a completed active Deep Debug archive. It retries deletion on every temporary-file exit, scavenges only stale exact-name entries under the automatic-report root, and deletes a Deep Debug archive only after a successful queue.

Product telemetry is a separate fixed-schema path. Core owns a discriminated per-kind allowlist and realistic value bounds; Runtime owns a redirect-disabled HTTPS transport to the exact official endpoint; App maps an allowlist of non-frame process observations into bounded events and holds at most 256 in memory. It has no disk retry queue and never includes arbitrary diagnostic data or causes PNG cloning. The versioned telemetry choice gates collection and delivery independently from online control and diagnostics.
...
- Private-server links and webhook URLs use DPAPI protection at rest and are redacted from diagnostics. The private-server field is intentionally visible while editing; the webhook remains masked. Webhook delivery accepts only HTTPS URLs on Discord's stable, PTB, or Canary hosts with the canonical `/api/webhooks/{id}/{token}` path. See [Privacy](../PRIVACY.md).
