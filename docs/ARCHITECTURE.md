...
Diagnostic transport remains separate from Deep Debug capture. Core owns the single 3 GiB archive limit, deterministic filename and kind validation, and trusted-storage policy. Runtime owns the WPF-free HTTPS, SHA-256, and multipart protocol plus a random installation-identity store. App owns the automatic-report choice, bounded in-memory light buffer, cancellation, and lifecycle disposal. Automatic reporting subscribes to frame observations only while enabled, packages at most 32 already-acquired frames into an 80 MiB temporary ZIP after an error, or submits a completed active Deep Debug archive. It retries deletion on every temporary-file exit, scavenges only stale exact-name entries under the automatic-report root, and deletes a Deep Debug archive only after a successful queue. The app has no manual diagnostic file picker or upload control.
