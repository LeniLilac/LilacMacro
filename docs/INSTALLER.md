...
Official builds create a fresh bundled CPU OCR runtime. The release workflow restores only the pinned pip downloads and official model files through a version-and-builder-keyed GitHub Actions cache, then assembles and validates a new runtime; it never restores the assembled-runtime directory. To keep local iteration practical, unsigned development builds may reuse a complete assembled Python/OCR runtime from `%LOCALAPPDATA%\LilacMacro\BuildCache\ocr` after matching the builder hash, Python version, and pinned Paddle versions. The caches are outside the repository and are never installed or published as mutable state.
...
An owner-approved in-app update downloads the exact six-asset release metadata, then downloads the manifest, signature, checksum, and installer from exact official GitHub URLs without a token. It requires GitHub's SHA-256 digest for every asset, verifies the Ed25519 manifest signature and its tag/name/size binding, requires the installer hash to match both the signed manifest and checksum, and rehashes the cached installer immediately before launch. Any missing, changed, redirected, private, draft, or disallowed prerelease artifact fails closed. Settings also exposes up to the newest 100 official releases in a version selector. That explicit download action applies the same verification chain, rehashes the copied file, and saves a standalone installer chosen by the owner; it never silently installs or downgrades the application.

The elevated installer creates a dedicated non-reparse ProgramData control directory, replaces inheritance with an Administrators/SYSTEM-write and Users-read ACL, and refuses an unsafe precreated request path. It rehashes its own source, records every configured runner with an active or disconnected logged-in Windows session, publishes a shutdown request that every owner/runner UI can observe, gives each UI five seconds to flush normally, then applies an exact four-product-image `taskkill` fallback before replacing the one shared Program Files installation. This avoids relying on cross-account process-handle inspection, which is not reliable across owner and runner Windows profiles. The in-app button first shows a LilacMacro confirmation, then starts this coordinated path after Windows UAC. The installer suppresses the setup wizard but keeps its non-cancelable native progress window visible until the app closes, upgrades, and relaunches; installation errors remain visible. Setup then validates and repairs configured runner tasks. Routine repair preserves active RDP viewports when the exact owned registry configuration is already applied and Remote Desktop Services is running; configuration drift, a stopped service, or a future native-path migration still requires the disruptive service restart. After successful repair, Setup relaunches every recorded runner UI, clears the request, and reopens the owner UI. A manually opened installer uses the same shutdown path but retains the ordinary setup wizard. Because Windows Restart Manager cannot close a detected process across sessions and treats the install-once TermWrap payload as owned by Remote Desktop Services, it is not used. Uninstall uses the same exact product list: it requests a non-force close, waits five seconds for UI flush, then applies the bounded force-close fallback before runner cleanup. It aborts on any unexpected shutdown result and never terminates Roblox, Remote Desktop Services, or unrelated processes.
