...
- At Main Macro plan start and every private-server Lobby reset, the owning Windows session first closes its Roblox client, waits for process exit, normalizes the current profile's `GlobalBasicSettings_13.xml`, converts the validated private-server share/link code to a registered `roblox://` target, and launches Roblox directly without opening a browser. The Roblox desktop client can turn the first ordinary close request into its own confirmation surface, so the lifecycle rediscovers current-session clients before a second bounded close request and before each of two bounded process-tree termination passes. This covers Roblox's observed PID handoff during close; clients in other Windows sessions are not targeted.
