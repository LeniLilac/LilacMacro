...
- At Main Macro plan start and every private-server Lobby reset, the owning Windows session first closes its Roblox client, waits for process exit, normalizes the current profile's `GlobalBasicSettings_13.xml`, and only then opens the validated private-server link. Graceful close receives a bounded opportunity before the exact current-session client is terminated; clients in other Windows sessions are not targeted.
...
- Main Macro, the local-session runtime, and Runtime Lab Wire Test always normalize rendered UI scale before task navigation. Main Macro and the local-session runtime repeat it after every restarted private-server join. Runtime Lab Debug exposes the identical rendered-scale operation for owner testing. There is no setting that disables normalization or fresh Lobby verification.
