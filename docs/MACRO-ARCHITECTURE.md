...
4. quarantine the failing task with an inspectable reason, continue with the next eligible task, and reconsider quarantined Utility work only at legitimate non-quarantined task handoffs;
...
The current Prototype preflights every configured task and placement route before OCR startup, Discord run-start delivery, or runtime input begins. Expected authoring omissions stop locally as `preflight_blocked` with the exact task and corrective setup action; they do not become a macro runtime error or automatic diagnostic upload. Unexpected storage, permission, or infrastructure faults still pass through the terminal runtime-error path. A bounded placement selection-proof miss is handled inside the match: the target is freshly reselected at most three times, then the authored step is logged as skipped and the next step continues without restarting Roblox. A safe-dismissal failure, capture/input failure, invalid state, or other operational exception returns to the scheduler instead of ending the Macro: retries wait 2, 5, 15, then at most 30 seconds; the reset path restarts Roblox, reapplies normalization, rejoins the private server, and verifies Lobby. Desktop recovery continuations preserve the dashboard's owning synchronization context before invoking UI-backed callbacks or starting another attempt. Three consecutive failures attributed to a match task quarantine it for 5 minutes, allowing another eligible task to run. If that task is inside a partially completed loop, quarantine abandons only the current containing-loop iteration: bounded descendant victory and defeat counters reset, nested iteration counters reset, the containing loop's completed-run count is preserved, and scheduling restarts at its first eligible child. Three consecutive failures attributed to a Utility task quarantine it indefinitely, remove it from normal priority selection, and allow one opportunistic retry only when the Macro is already handing off to a different non-quarantined task at Lobby; a failed opportunistic retry remains quarantined and the next task continues, while a successful retry clears the quarantine. Managed-session execution uses the same capped 2/5/15/30-second operational retry ladder around a fresh full-plan reset, while invalid serialized configuration remains terminal. Each recovery, quarantine, and abandoned loop iteration is written to the run log and deep-debug events. Owner live acceptance and broader anomaly classification remain required before this objective can be promoted beyond Prototype.
...
Lobby -> Play UI -> Tower -> configured Trait Tower or Traitless Tower -> topmost/rightmost highest-floor text -> Tower stage Select Stage verification -> Match Preview map/floor recognition -> goal-already-cleared Lobby reset or Match Prestart -> load that Story map's Tower-route team -> camera/map preparation -> placements -> Start Game -> Victory/Defeat.

Tower uses separate `trait-tower` and `traitless-tower` placement routes on every Story map. The stage modal owns Select Stage, while a distinct three-scale Match Preview ROI owns the map and available floor; team selection therefore occurs after Match Prestart. If the available floor is above the configured goal, the prior floor is recorded as already cleared and the runtime uses the shared private-server restart/rejoin path without starting the match. An available floor equal to the goal still runs. A verified Victory stores the cleared floor as progress and also restarts/rejoins so the next pass can select the newly available highest floor. A Defeat below the configured per-floor limit clicks verified Repeat Floor and resumes from fresh Match Prestart without another team load or camera alignment. The default limit stops on the fifth defeat on one floor, and any Victory resets it.
...
The node loop locates and hovers only the current marker. A stable tooltip title owns semantic calibration, while the color learned from that observation is a per-environment hot path with hover-OCR fallback and refresh. Future-node lookahead is not required. Defense and Elite wait for Start Game before replaying placement/configuration. The game moves physical units into the node area and deletes phantoms. Once a replay probe shows no selection UI, that placement is retained as physical and omitted from later Defense/Elite replay probes for the rest of the match; any selected-unit panel proves that replay placement succeeded as either phantom or affordable physical, so it receives saved configuration and remains eligible for later replay. Assault and Boss wait; Encounter and non-spawn Checkpoint wait for ship arrival and then use separate verified Continue source/modal states; Checkpoint applies extraction policy. The detailed field contract and unresolved behavior are in [Expedition runtime](EXPEDITION-RUNTIME.md).
...
**Prototype:** Plan edits, selection, Discord event options, and the independent default-on automatic Deep Debug error-upload choice autosave through a serialized queue to a schema-versioned atomic settings file; invalid plan payloads fail closed to the built-in defaults without discarding separately valid settings. Private-server and webhook values are masked in Settings and persisted only as current-user DPAPI ciphertext. Validated web/share links are reduced to bounded share/link codes and launched through the registered `roblox://` protocol without a browser intermediary. Test Webhook performs a bounded, mention-free text-only delivery to the validated Discord endpoint. Runtime events capture the current Roblox client at the event boundary, queue network delivery in the background, and fall back to text if local capture is unavailable. Terminal failure notifications always include failure details and are the only event allowed to mention the configured user. Persisted secrets remain redacted from diagnostics, logs, tests, and captures. Automatic reports send only completed Deep Debug archives containing classified failures; sent and failed archives remain local until the shared byte budget evicts the oldest evidence, and no manual upload surface exists.

The Main Macro also atomically persists per-plan runtime progress in an instance-scoped progress file. Victory/defeat counters, Forever/finite loop completion counts, and utility due timestamps are restored after Macro Stop/Start and application close/reopen. The elapsed timer and run chart intentionally reset at every Macro start. RESET PROGRESS is available only while stopped and clears both in-memory and saved progress. Runtime progress is keyed by persisted plan/task/loop identities; changing a plan identity creates a new progress record. **Planned:** explicit recovery UX for DPAPI data that cannot be decrypted.
...
- Remaining eligibility/cooldown policies and scheduler completion boundaries
