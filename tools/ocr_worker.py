...
def run_ocr(
    ...
    results = list(cache[cache_key].predict(input=str(input_path)))
...
def serve(channel_value: str) -> int:
...
def main() -> int:
    ...
    if args.serve:
        return serve(str(args.channel))
    payload = run_ocr(str(args.input), str(args.model), str(args.device), {})
