...
public sealed class TermServiceConfigurationManager(LocalSessionPaths paths)
{
    ...
    public void Restart()
    {
        ...
        using ServiceHandle service = OpenService(scm, "TermService", 0x0010 | 0x0020 | 0x0004);
        if (service.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(), "TermService could not be opened.");
        ServiceStatus status = default;
        _ = ControlService(service, 1, ref status);
        WaitForState(service, 1, TimeSpan.FromSeconds(20));
        if (!StartService(service, 0, null) && Marshal.GetLastWin32Error() != 1056)
            throw new Win32Exception(Marshal.GetLastWin32Error(), "TermService could not be started.");
        WaitForState(service, 4, TimeSpan.FromSeconds(20));
    }

    private static void WaitForState(ServiceHandle service, uint desiredState, TimeSpan timeout)
    {
        ...
        throw new TimeoutException($"TermService did not reach state {desiredState}.");
    }
