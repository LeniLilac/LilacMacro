...
When online features are enabled, LilacMacro polls the signed public control snapshot, obtains public game codes and maintenance/feature-disable state, checks official GitHub release metadata when configured, and may use service-backed configuration sharing when that feature is available. Turning the choice off prevents new LilacMacro service and update-check requests; one bounded request already in flight can finish and update its local public cache. Local automation remains available without current remote safety status.
