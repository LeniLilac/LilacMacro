...
**Status: Implemented behavior. Notice version 1, effective August 15, 2026.**
...
When online features are enabled, LilacMacro polls the signed public control snapshot, obtains public game codes and maintenance/feature-disable state, checks official GitHub release metadata when configured, and permits explicit configuration sharing. Export sends one compressed opaque bundle containing only the selected Plan and/or saved placement documents to LilacMacro Services over trusted HTTPS. It excludes private-server links, Discord settings, privacy choices, run history, diagnostics, and other application settings. The service returns a random 20-character bearer code, stores only its SHA-256 hash, retains the opaque payload for at most 30 days, stores a separate payload-integrity hash, and uses a monthly rotating share-specific network pseudonym only for daily abuse limits. Anyone with the code can retrieve the bundle until it expires, so users should share codes privately. Retrieval puts the code in a no-store JSON request body rather than a logged URL. Import always requires an explicit code and component selection, is rejected while another Macro uses that configuration, and rolls back staged placement replacements if the combined import fails. Imported Plans are added without replacing an existing Plan, while selected placement documents replace matching local map files. Turning the online-features choice off prevents new LilacMacro service and update-check requests; one bounded request already in flight can finish and update its local public cache. Local automation remains available without current remote safety status.
