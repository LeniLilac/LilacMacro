...
- Full layout opens at `1920 x 1080` and exposes one compact left-aligned Dock/Undock action. Dock does not maximize the shell and presents the client at exactly `1366 x 700` physical pixels inside a fixed `766`-pixel dashboard workspace. The Stats and Upcoming Tasks inspector has that same exact height; its task viewport clips overflow instead of enlarging either side. Full-layout minimum sizing preserves the complete dock and inspector, while the lower run log may collapse out of view. While Dock remains enabled, a Roblox window reopened in the same desktop is reacquired automatically, including when the reopened client becomes foreground. Compact layout opens at `1366 x 768`, removes the dock surface and action entirely, and always minimizes the macro while a run owns the desktop. Undocking or any automatic dock release leaves Roblox undocked at its last docked position and size so the owner can reposition or resize it; closing restores the standalone window styles at that same geometry.
...
- General shows the running Macro version and the stable random installation ID as terse read-only values at the top of Updates and data. The installation ID has one adjacent Copy action so a user can provide it for an administrator's exact diagnostic search without opening local files.
...
- Diagnostics owns a `Deep Debug` group with default-on logging, fixed one-second capture, classified error windows spanning ten seconds before and after each failure, representative transition frames, and one storage budget shared across provisioned owner/runner profiles. The default budget is selected from free-space tiers (3, 10, or 30 GiB), and capture pauses below 3 GiB free. Automatic Deep Debug error uploads default on, pause whenever Deep Debug is unavailable, and have no light-report fallback. There are no redundant switches or persistent status row; completed archive-save feedback uses the shared bottom-right notification. Recording and manual diagnostic-upload surfaces remain absent.
- Controls are session-only prototypes unless their current implementation explicitly says otherwise. Theme mode and color-family selection update live and persist with display/update preferences, Auto/GPU/CPU OCR mode, press-then-key bindings, private-server/webhook values, Discord failure options, Plan state, and local-instance profiles across versioned artifacts. The OCR tab owns a local generated-image test and selected-runtime repair. Every macro UI runs on its own desktop; there is no run-target selector. Current Story/Raid Play and Unit inventory bindings may be unset to use verified OCR button navigation. Areas exposes the same optional binding contract for its future runner. Webhook delivery, private-server protocol launch, coordinated updates, and automatic deep-debug diagnostics are connected.
